...
class Message:
    def __init__(self, packet):
        ...
        print("initialization packet", self.cid, self.cmd, format_bytes(self.data))

    def process_cont_packet(self, packet):
        if self.done:
            raise Exception("Continuation packet sent when message is already done")
        
        cid = int.from_bytes(packet[:4], "big")
        if cid != self.cid:
            raise Exception(f"Cid doesn't match: initial {self.cid}, current {cid}")
        ...
        self.data[self.data_cursor:self.data_cursor + data_end] = data[:data_end]
        self.data_cursor += len(data)
        self.done = self.data_cursor == self.data_len
        if not self.done:
            self.next_cont_packet = seq + 1
        print("continuation packet", self.cid, self.cmd, format_bytes(self.data))